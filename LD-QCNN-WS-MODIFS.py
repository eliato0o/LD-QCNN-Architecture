...
def extract_quantum_parameters_from_unitary(target_unitary, num_params=32):
    ...
    print(f"      -> Synthèse du bloc terminée (Loss Frobenius: {final_loss:.4f})")
...
# ============================================================================
# 3. PRÉPARATION DES DONNÉES (MODIFIÉE AVEC LE SAMPLER)
# ============================================================================
def load_and_prepare_data(data_dir='./brain_tumor_dataset', num_per_class=1500):
    ...
    # 1. Séparer les indices par classe
    ...
    # 2. Déterminer le nombre d'images à extraire (1500 maximum, ou moins si le dataset est petit)
    ...
    # 3. Tirage aléatoire (seed fixe pour que les comparaisons entre tes tests soient justes)
    random.seed(42) 
    subset_0 = random.sample(class_0_indices, n_samples)
    subset_1 = random.sample(class_1_indices, n_samples)
    
    # 4. Création du dataset réduit et parfaitement équilibré
    ...
    # 6. DataLoaders : le sous-ensemble étant 50/50, un shuffle classique suffit
...
def main():
    ...
    # --- MODIFICATION DE LA LOSS ---
    # Plus besoin de pos_weight, le dataset vu par le réseau est parfaitement équilibré (50/50)
